...
export function DefensiveAggregate({ entries, spellId, occurrence, onInspect, onOpenPull, allMitigation, showReady, onShowReady, bossIds, onBossIds }) {
  ...
  const points = displayDamagePoints(rows.flatMap((r) => r.player.pressure), allMitigation);
  ...
  return <>
    ...
    <div className="coverage-scroll" tabIndex="0" role="region" aria-label="Defensive usage across pulls">
      <div className="coverage-canvas" style={{ minWidth: 1000 }}>
        ...
        {rows.length > 0 && <DamageLegend points={points} maximum={peak} allMitigation={allMitigation} />}
        <TimelineLegend showReady={showReady} casts={rows.flatMap((row) => row.casts)} deaths={rows.flatMap((row) => row.player.deaths)} />
        {rows.map(({ pull, player, casts, anchor }) => <div className="coverage-row defensive-aggregate-row" key={pull.id}>
          ...
          <div className="defensive-aggregate-track">
            <div className="defensive-mini-pressure" style={{ left: `${(-anchor - start) / duration * 100}%`, width: `${pull.duration / duration * 100}%` }}><PersonalDamageGraph points={displayDamagePoints(player.pressure, allMitigation)} duration={pull.duration} maximum={peak} /></div>
            ...
            <DefensiveTrack showReady={showReady} casts={casts} duration={pull.duration} offset={anchor} start={start} end={end} onInspect={onInspect} />
