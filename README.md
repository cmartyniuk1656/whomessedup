...
- `nymrissa-wavecaller-mythic-mechanics` reviews Frost Orb pops during the observed Abyssal Rain channel, with a one-second buffer before and after; the lingering DoT is excluded. `nymrissa_mechanics*` separates fetching, pure event analysis and vocabulary; the renderer reuses compact mechanics tables and player bars. See [the sample audit and timing rules](docs/analysis/nymrissa-mythic.md).
