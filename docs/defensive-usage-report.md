...
### Additional tracked abilities and log mappings

`supplemental-abilities.json` adds Priest Fade, Bear Form for Balance/Feral/Restoration, and Warrior Defensive Stance without rewriting the original research package. September 24 tooltip and talent-tree checks establish Fade's 30-second base cooldown, Improved Fade's 5/10-second reduction, Translucent Image's 10% reduction, and Defensive Stance's 15% base reduction. Bear Form's armor/health benefits remain unvalued. Stances use observed buff entries and exits, including entries caused by another action, continue to fight end when no exit is recorded, and stop at death; they have no cooldown-readiness diamonds. Guardian Druids are excluded from Bear Form tracking. No pre-pull activation is inferred without an observed entry.

Sources: [Fade](https://nether.wowhead.com/tooltip/spell/586), [Translucent Image](https://nether.wowhead.com/tooltip/spell/373446), [Improved Fade](https://nether.wowhead.com/tooltip/spell/390670), [Bear Form](https://nether.wowhead.com/tooltip/spell/5487), [Defensive Stance](https://nether.wowhead.com/tooltip/spell/386208), and [talent definitions](https://nether.wowhead.com/data/talents-dragonflight). New talent entries are extracted by the existing pinned SimulationCraft trait-map script. Fight Through the Flames makes stance reduction school-dependent and withholds a fixed-rate estimate; Unyielding Stance's unconditional increase is supported.

`defensive_events.py` normalizes the verified Guardian of Ancient Kings cast/buff ID `212641` to catalog ID `86659`. Environment-targeted personal casts still belong to the caster. Aura-only Guardian procs are not counted as manual casts. Paired auras allow 100ms of earlier event ordering because real form applications can precede their cast timestamp by 1ms.

Across-pull summary cards merge Holy and Retribution Divine Protection (498/403876), counting each pull once while leaving individual spell IDs, talent adjustments and recharge estimates intact. The reported example is Malandru changing specs: 34 Holy uses plus 4 Retribution uses, now one 38-use card across 14 pulls. Cache version 9 forces refreshed report data for the new mappings.

Live event validation covered 25 pulls: 21 recorded Guardian casts (5 Sentinels / 16 Nymrissa), 167 Fades, 99 non-Guardian Bear Form entries and 12 Defensive Stance entries. Guardian aura-only procs remain excluded. Regression tests cover alias deduplication, environment recipients, stance deaths/end-of-pull, talent gates, Fade recharge ranks and summary grouping across spec changes.
