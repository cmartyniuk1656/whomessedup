...
15. Real-time mode uses a lightweight, shared report-metadata poll. A matching fight must have an unchanged end time across two polls before it triggers a fresh background job; the current page remains visible and is replaced atomically when that job completes. Revisions without a new fight trigger a full correctness refresh.
16. Bosses with at least two visible non-cooldown reports expose a generated Aggregate Reports definition. Its request fields namespace child-report settings, and its result embeds the unchanged child page models behind a report-level selector instead of merging unrelated table schemas.
