...
def _register_coverage_reports():
    ...
    for encounter_id, boss in bosses.items():
        for difficulty in (ReportDifficulty.HEROIC, ReportDifficulty.MYTHIC):
            ...
            report_id = f"{fight_id}-cooldown-coverage-{difficulty.value}"
            _REPORTS[report_id] = RegisteredReport(
                definition=ReportDefinitionModel(
                    id=report_id, title="Cooldown Coverage Report",
                    description="Explore healer cooldown windows, boss casts and incoming raid damage across every pull.",
                    fightId=fight_id, fightName=boss["name"], defaultFight=boss["name"], difficulty=difficulty,
                    requestSchema=RequestSchemaModel(fields=[_build_report_codes_field(), RequestFieldModel(
                        id="fresh_run", kind=RequestFieldKind.CHECKBOX, label="Force fresh run (skip cache)", defaultValue=False)])),
                job_type=JOB_V2_COOLDOWN_COVERAGE,
                build_payload=payload_builder(report_id, encounter_id, difficulty))
...
def _register_defensive_reports():
    ...
    def payload_builder(report_id, encounter_id, difficulty):
        def build(values):
            return {"report_id": report_id, "report_codes": _coerce_report_code_list(values),
                    "encounter_id": encounter_id, "difficulty": difficulty.value, "defensive_version": 9}, _coerce_bool(values, "fresh_run", default=False)
        return build
